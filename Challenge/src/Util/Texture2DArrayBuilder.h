...
public:
    ...
    gl::Texture build(std::uint32_t width, std::uint32_t height) const noexcept;
    /*
        Return value: the index of the texture array.
     */
    std::uint32_t append(Image&& image) noexcept;
private:
    static Image scaleImage(const Image& image, std::uint32_t width, std::uint32_t height) noexcept;
